...
def _instance_names(font: TTFont) -> tuple[frozenset[int], dict[int, int]]:
    ...
    the family name would leave a font whose styles are all called "Owl Mono".
    Its PostScript name, on the other hand, is a full name and does carry the
    family.
    """
    keep: set[int] = set()
...
def rename(path: str, family: str, original: str) -> None:
    ...
    for record in list(name.names):
        ...
        if nid in postscript_ids:
            ...
            name.setName(f"{family};owl-transfer", nid, *where)
