...
//   app/public/favicon.svg                           the same tile at 32 units, for the browser tab
...
// and `favicon.svg` back and asserts they still carry these exact path strings
// and eye coordinates, so the drawing lives here once and nowhere else.
...
/**
 * The browser tab, which is the launcher tile drawn at grid size.
 *
 * An amber ground with the face cut out of it rather than amber strokes on
 * nothing: a tab favicon is 16px on whatever colour the browser chrome is, and
 * a thin amber line on a light toolbar is not there at all. The tile carries
 * its own corners, `rx` 7 of 32, which is the desktop icon's 225 of 1024.
 *
 * Laid out by hand rather than through `face()`, because at this size the
 * shared attributes read better hoisted onto two groups than repeated on every
 * element. The strings are still the same strings, and `marks.test.ts` reads
 * the written file back to check that they are.
 */
const FAVICON_RADIUS = 7;
const faviconSvg = `<!-- The mark from app/src/icons/OwlMark.tsx on the launcher's amber ground.
     The paths are the same strings; scripts/render-icons.mjs reads them too. -->
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="32" height="32">
  <rect width="32" height="32" rx="${FAVICON_RADIUS}" fill="${AMBER}" />
  <g fill="none" stroke="${INK}" stroke-width="${STROKE}">
    <path d="${BROW}" stroke-linecap="round" stroke-linejoin="round" />
${EYES.map(e => `    <circle cx="${e.cx}" cy="${e.cy}" r="${RING_R}" />`).join('\n')}
  </g>
  <g fill="${INK}">
${EYES.map(e => `    <circle cx="${e.cx}" cy="${e.cy}" r="${PUPIL_R}" />`).join('\n')}
    <path d="${BEAK}" />
  </g>
</svg>
`;
